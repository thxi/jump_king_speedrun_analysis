# idea taken from https://github.com/TobiasRoeddiger/GazePointHeatMap/blob/master/gazeheatplot.py
...
def draw_heatmap(gazepoints,
    ...
    # IMAGE
    fig, ax = draw_display(dispsize, img)
    ...
    for i in range(0, len(gazepoints)):
        ...
        if (not 0 < x < dispsize[0]) or (not 0 < y < dispsize[1]):
            ...
            try:
                heatmap[y:y + vadj[1], x:x +
                        hadj[1]] += gaus[vadj[0]:vadj[1],
                                         hadj[0]:hadj[1]] * gazepoints[i][2]
            except:
...
parser = argparse.ArgumentParser(
    description='Parameters required for processing.')

#required args
parser.add_argument('input-path', type=str, help='path to the csv input')
parser.add_argument('display-width',
                    type=int,
                    help='an integer representing the display width')
parser.add_argument('display-height',
                    type=int,
                    help='an integer representing the display height')
...
alpha = args['alpha']
output_name = args['output_name'] if args[
    'output_name'] is not None else 'output'
...
with open(input_path) as f:
    ...
    draw_heatmap(gaze_data, (display_width, display_height),
                 alpha=alpha,
